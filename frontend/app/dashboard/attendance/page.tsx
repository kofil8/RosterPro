    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Track employee clock-in/out times</p>
        </div>
                  ...
                  <div className="flex-1">
                    ...
                    {attendance.shift && (
                      <p className="text-sm text-gray-600 mb-2">
                        ...
                        {attendance.shift.clientName && ` - ${attendance.shift.clientName}`}
                      </p>
                    )}
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                      <div>
                        <p className="text-xs text-gray-500">Clock In</p>
                        <p className="font-medium">{formatDate(attendance.clockIn)}</p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-500">Clock Out</p>
                        <p className="font-medium">
                          {attendance.clockOut ? formatDate(attendance.clockOut) : 'Not clocked out'}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-500">Total Hours</p>
                        <p className="font-medium">{formatHours(attendance.totalHours)}</p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-500">Break Duration</p>
                        <p className="font-medium">{attendance.breakDuration.toFixed(2)} hrs</p>
                      </div>
                    </div>
                    {attendance.notes && (
